...
#define DATAFILE "/tmp/chance.data"
...
int get_player_data()
{
	...
	if(read_bytes < (int)sizeof(struct user))
		return -1;
	else
		player = entry;
	return 1;
...
void update_player_data()
{
	...
	{
		for(i = 0; i < (int)sizeof(struct user) - 4; i++)
...
void show_highscore()
{
	...
	while(read(fd, &entry, sizeof(struct user)) > 0)
		if(entry.highscore > (int)top_score)
		{
			top_score = entry.highscore;
			strcpy(top_name, entry.name);
		}
	close(fd);
	if((int)top_score > player.highscore)
